...
// Modified by: Darnell Chambers Gordon
// Comment file
 ...
 */
template <typename Comparable, typename Comparator>
void quickMiddle( vector<Comparable> & a, int left, int right, Comparator less_than ){
    if( left + 10 <= right )
    {
        ...
        while (i <= j)
        {
            while( less_than(a[ i ], pivot) ) { i++; }
            while( less_than(pivot, a[ j ]) ) { j--; }
            if( i <= j ) {
                std::swap(a[i], a[j]);
                i++;
                j--;
        ...
        if (left < j)
            quickMiddle( a, left, j, less_than );     // Sort small elements
        if (i < right)
            quickMiddle( a, i, right, less_than );    // Sort large elements
    }
    else  // Do an insertion sort on the subarray
        insertionSort( a, left, right, less_than );
}
 ...
 */
template <typename Comparable, typename Comparator>
void quickFirst( vector<Comparable> & a, int left, int right, Comparator less_than ){
    if( left + 10 <= right )
    {
        ...
        if (left < j)
            quickFirst( a, left, j, less_than );     // Sort small elements
        if (i < right)
            quickFirst( a, i, right, less_than );    // Sort large elements
    }
    else  // Do an insertion sort on the subarray
        insertionSort( a, left, right, less_than );
}
...
void QuickSort2(vector<Comparable> &a, Comparator less_than) {
  ...
  // to be filled
    quickMiddle(a, 0, a.size() - 1, less_than);
}
...
void QuickSort3(vector<Comparable> &a, Comparator less_than) {
  // quicksort implementation
  // to be filled
    quickFirst(a, 0, a.size() - 1, less_than);
}
